...
using PaperUtils
using Plot, UCDColors, SimpleStats, PairsDB, RelayGLM, Progress
...
function make_figure(d::Dict{String,Any}, ex_id::Integer=208)
    ...
    labels = Dict("msequence"=>"Binary white noise", "grating"=>"Gratings")
    ...
    for type in ["msequence", "grating"]
        ...
        y = d[type]["xf"][:, k]
        se = d[type]["xse"][:, k]
        plot_with_error(x, y, se, RGB(colors[type]...), ax[2,1], linewidth=3, label=labels[type])
    ...
    ax[1,1].set_title("ISI-efficacy: Pair ID $(ex_id)", fontsize=16)
    ...
    ax[2,1].set_title("RH-retinal: Pair ID $(ex_id)", fontsize=16)
